...
inline bool cpuid(uint32_t op, uint32_t sub_op, uint32_t * res) noexcept /// NOLINT
{
#if defined(__x86_64__)
...
inline bool cpuid(uint32_t op, uint32_t * res) noexcept /// NOLINT
{
#if defined(__x86_64__)
...
union CPUInfo
{
    ...
    explicit CPUInfo(uint32_t op) noexcept { cpuid(op, info); }

    CPUInfo(uint32_t op, uint32_t sub_op) noexcept { cpuid(op, sub_op, info); }
};
